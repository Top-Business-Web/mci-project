...
@section('content')
            ...
            <div class="row">
                <div class="col-12 col-lg-6">
                    @foreach ($product->images as $image)
                        <div class="product-img">
                            <img class="img-fluid w-100" src="{{ asset($image) }}" alt="product image"/>
                    ...
                    {{--                        @endforeach--}}
                    {{--                    </div>--}}
                    {{--                    <div class="small-slider slider-right">--}}
                    {{--                        @foreach ($product->images as $image)--}}
                ...
                <div class="col-12 col-lg-6">
                            ...
                            <table class="table w-100">
                                ...
                                @if($product->details)
                                    @foreach($product->details as $detail)
                                        <tr>
                                            <td class="name">{{ trans('site.part_number')  }}:</td>
                                            <td class="value">
                                                {{ $detail }}
                                            </td>
                                        </tr>
                                ...
                                <tr>
                                    ...
                                    <td class="value">{{ ($product->tags == null) ? '' : implode(' , ', $product->tags) }}</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="container mt-5">
                        <div class="row">
                            <div class="col-4">
                                <form action="{{ route('quoteIndex') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $product->id }}">
                                    <button type="submit"
                                            class="btn btn-sm btn--secondary">
                                        <i class="fa-solid fa-shop"></i>
                                        {{ lang() == 'ar' ? 'اطلب الان' : 'Order Now' }}
                                    </button>
                                </form>
                            </div>
                            <div class="col-4">
                                <a target="_blank"
                                   href="https://wa.me/+{{ $settings->whatsapp }}?text=اريد الاستفسار عن المنتج [{{ $product->title_ar }}]">
                                    <button type="button"
                                            class="btn btn-sm btn-success">
                                        <i class="fa-brands fa-whatsapp"></i>
                                        {{ lang() == 'ar' ? 'اطلب الان واتساب' : 'Order by Whatsapp ' }}</button>
                                </a>
                            </div>
                            <div class="col-4">
                                <a href="mailto:{{ $settings->email }}?subject=استفسار عن منتج&body=اريد الاستفسار عن المنتج [{{ $product->title_ar }}]">
                                    <button type="button"
                                            class="btn btn-sm btn-warning">
                                        <i class="fa-solid fa-envelope"></i>
                                        {{ lang() == 'ar' ? 'اطلب عن طريق الايميل' : 'Order by email' }}</button>
                                </a>
                            </div>
                        </div>
                    </div>
    ...
    <section class="shop shop-2">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <h5>@lang('site.related products')</h5>
                </div>
            </div>
            <div class="row">
